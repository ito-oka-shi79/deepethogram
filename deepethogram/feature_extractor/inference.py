...
def print_debug_statement(
    ...
    log.info(
        "spatial: min {} mean {} max {} shape {}".format(
            spatial_features.min(), spatial_features.mean(), spatial_features.max(), spatial_features.shape
        )
    )
    log.info(
        "flow   : min {} mean {} max {} shape {}".format(
            flow_features.min(), flow_features.mean(), flow_features.max(), flow_features.shape
        )
    )
...
def predict_single_video(
    ...
    for i, batch in enumerate(tqdm(dataloader, leave=False)):
        ...
        if i == 0:
            ...
            buffer["spatial_features"] = torch.zeros(
                (video_frame_num, spatial_features.shape[1]), dtype=spatial_features.dtype
            )
    ...
    for i in tqdm(range(len(rgbs))):
        ...
        if i == 0:
            for k, v in outputs.items():
                ...
                if k == "debug":
                    log.debug(
                        "All should be 1.0: min: {:.4f} mean {:.4f} max {:.4f}".format(v.min(), v.mean(), v.max())
                    )
